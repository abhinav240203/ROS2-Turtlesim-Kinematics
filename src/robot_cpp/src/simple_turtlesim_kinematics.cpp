#include "robot_cpp/simple_turtlesim_kinematics.hpp"
...
void SimpleTurtlesimKinematics::turtle2PoseCallback(const turtlesim::msg::Pose & pose)
{
   ...
   float theta_diff = last_turtle2_pose_.theta - last_turtle1_pose_.theta;
   float theta_deg = 180 * theta_diff/ 3.14;
   ...
   RCLCPP_INFO_STREAM(this->get_logger(),
    ...
    "theta(rad): " << theta_diff << "\n" <<
    "theta(deg): " << theta_deg << "\n" <<
    "|R11    R12|:  |" << std::cos(theta_diff) << "\t" << -std::sin(theta_diff) << "|\n" <<
    "|R21    R22|:  |" << std::sin(theta_diff) << "\t" <<  std::cos(theta_diff) << "|\n");
